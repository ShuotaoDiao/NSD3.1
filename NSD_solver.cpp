...
std::vector<double> nsd_presolve(standardTwoStageParameters& model_parameters, const secondStageRHSpoint& rhs, const secondStageRHSmap& RHSmap) {
    ...
    IloEnv env;
    IloModel mod(env);
    IloNumVarArray x_temp(env,model_parameters.c.getLen(),model_parameters.x_lb,IloInfinity,ILOFLOAT);
    IloNumVarArray y(env,model_parameters.D.getColLength(),0,IloInfinity,ILOFLOAT); // y >= 0
    mod.add(x_temp);
    mod.add(y);
    ...
    for (int col_idx = 0; col_idx < model_parameters.C.getColLength(); ++col_idx) {
        ...
        for (int idx = beg_idx; idx < model_parameters.C.getClen(col_idx) + beg_idx; ++idx) {
            exprs_eq[model_parameters.C.getRow(idx)] += model_parameters.C.getVal(idx) * x_temp[col_idx];
        }
    }
    // right hand side e
    ...
    for (int idx = 0; idx < rhs.Ce.size(); ++idx) {
        exprs_eq[RHSmap.Ce_map[idx].first] += rhs.Ce[idx] * x_temp[RHSmap.Ce_map[idx].second];
    }
    // coefficients before x (stochastic part) inequality (location is behind equality constraints)
    for (int idx = 0; idx < rhs.Ci.size(); ++idx) {
        exprs_eq[RHSmap.Ci_map[idx].first + model_parameters.num_eq] += rhs.Ci[idx] * x_temp[RHSmap.Ci_map[idx].second];
    }
    // right hand side (stochastic part) equality be_(i) equality
    for (int idx = 0; idx < rhs.be.size(); ++idx) {
        exprs_eq[RHSmap.be_map[idx]] -= rhs.be[idx];
    }
    // right hand side (stochastic part) equality bi_(i) inequality
    for (int idx = 0; idx < rhs.bi.size(); ++idx) {
        exprs_eq[RHSmap.bi_map[idx] + model_parameters.num_eq] -= rhs.bi[idx];
    }
    // add the equality constraints
    for (int idx = 0; idx< model_parameters.D.getRowLength(); ++idx) {
        constraintsEquality.add(exprs_eq[idx] == 0);
    }
    ...
    //cplex.exportModel("/Users/sonny/Documents/numericalExperiment/SDkNN/twoStageShipment/experiment6/presolve_model.lp");
    IloBool solvable_flag = cplex.solve();
    if (solvable_flag != IloTrue) {
        throw std::logic_error("nsd_presolve() Error: Master problem is infeasible.\n");
    }
    // return initial estimate
    std::vector<double> x_candidate(model_parameters.c.getLen(),0.0);
    for (int idx = 0; idx < x_candidate.size(); ++idx) {
        x_candidate[idx] = cplex.getValue(x_temp[idx]);
    }
...
solverOutput nsd_solver(const std::string& folder_path,
    ...
    std::string resultsOutput_path = folder_path + "/computationalResults(sdknn3.1).txt";
    ...
    std::cout << "*******************************************\n";
    writeFile << "*******************************************\n";
    std::cout << "SD-kNN (v3.0) is initialized\n";
    writeFile << "SD-kNN (v3.0) is initialized\n";
    ...
    for (int iteration = 0; iteration < max_iterations; ++iteration) {
        ...
        //std::cout << "Debug: Finish RHS calculation\n";
        // update the minorant
        //new_minorant_collection.clear();
        // l1 norm
        //minorant_l1norm.clear();
        ...
        for (int index = 0; index < k_new; ++index) {
            ...
            for (int dual_index = 0; dual_index < explored_duals.size(); ++dual_index) {
                ...
                if (iteration == 0) { // first iterationn
                    double sto_pi_e = 0;
            ...
            } // end for loop for dual selection
            /*
...
// NSD solver of sampling a sequence of decisions
...
solverOutput nsd_solver2(const std::string& folder_path,
    ...
    std::string sto_path = folder_path + "/sto.txt";
    std::string resultsOutput_path = folder_path + "/computationalResults(sdknn3.1).txt";
    std::string resultsSolution_path = folder_path + "/sol(sdknn_v3.1).txt";
    // convert all the paths into constant chars
    const char* be_DB_path_const = be_DB_path.c_str();
    const char* bi_DB_path_const = bi_DB_path.c_str();
    const char* Ce_DB_path_const = Ce_DB_path.c_str();
    ...
    std::cout << "*******************************************\n";
    writeFile << "*******************************************\n";
    std::cout << "SD-kNN (v3.1) is initialized\n";
    writeFile << "SD-kNN (v3.1) is initialized\n";
    std::cout << "Algorithmic Parameters\n";
    writeFile << "Algorithmic Parameters\n";
    std::cout << "sigma, q, beta, k, N, N_pre, sigma_lower, sigma_upper" << std::endl;
    writeFile << "sigma, q, beta, k, N, N_pre, sigma_lower, sigma_upper" << std::endl;
    ...
    for (int iteration = 0; iteration < max_iterations; ++iteration) {
        ...
        if (k == k_new && old_knn_radius <= distance_squared) { // kNN set radius is not changed, new one does not enter the kNN set
            ...
            kNNSet.clear(); // clear the old kNN set
            newkNNSet.clear();
            // *remove the intermediate value for the duaks, if k does not increase*
            if (k == k_new && iteration > 0) {
                for (int idx = 0; idx < explored_duals.size(); ++idx) {
                    explored_duals[idx].sto_pi_e.erase(explored_duals[idx].sto_pi_e.begin() + rel_kpos); // clear the memory of the pi_e for the old kth NN
                }
